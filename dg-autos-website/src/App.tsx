...
const paintPackages = [
  { id: "daily", name: "Daily Driver Refresh", price: "$3,500+", label: "Clean & presentable", text: "A practical full-body repaint for a vehicle you use every day. Built around a clean, uniform finish without paying for show-car levels of disassembly and correction.", includes: ["Same-color exterior baseline", "Standard prep & masking", "Minor surface correction", "Base/clear or appropriate paint system", "Final finish inspection"] },
  { id: "dealer", name: "Dealership Finish", price: "$5,500+", label: "Factory-style appearance", text: "For customers who want the vehicle to present closer to a newer dealership or factory-style finish, with more preparation, disassembly, and finish refinement.", includes: ["More extensive prep", "Additional trim/component removal as needed", "Improved panel & surface correction", "Higher finish refinement", "Finish polish/detail as required"] },
  { id: "show", name: "Show Quality", price: "$8,500+", label: "Highest finish standard", text: "For enthusiast, restoration, weekend, and show-oriented vehicles where preparation and final appearance matter more than keeping the project at a basic repaint budget.", includes: ["Extensive prep & blocking", "Greater disassembly", "Detailed surface correction", "Premium finish attention", "Cut/polish & final refinement"] },
];
...
const paintBudgets = ["$3,500–$4,999", "$5,000–$7,499", "$7,500–$9,999", "$10,000+", "Financing will help determine my budget", "Not sure yet"];
...
function App() {
  ...
  const [selectedPaintPackage, setSelectedPaintPackage] = useState("dealer");
  ...
  async function submitAppointment(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      form.reset(); setAppointmentService("Full Body Paint Consultation"); setSelectedPaintPackage("dealer"); setAppointmentDate(""); setAvailableSlots([]);
  ...
  return <div className="site-shell">
    ...
    <main>
      ...
      <div className="paint-package-section"><div className="paint-package-heading"><p className="eyebrow">Planning packages</p><h3>Know the starting point before you book</h3><p>These are starting-price baselines so you can decide what level of finish makes sense for your vehicle. Your final estimate is built after we inspect the car and confirm the actual prep, bodywork, color, disassembly, and finish requirements.</p></div><div className="public-package-grid">{paintPackages.map(pkg=><article key={pkg.id} className={`public-package-card ${pkg.id==="dealer"?"featured":""}`}><small>{pkg.label}</small><h4>{pkg.name}</h4><strong>{pkg.price}</strong><p>{pkg.text}</p><ul>{pkg.includes.map(item=><li key={item}>{item}</li>)}</ul><a href="#booking">Choose this level →</a></article>)}</div><p className="package-disclaimer">Starting prices assume an average-size vehicle with a same-color exterior repaint and no major rust or collision repair. Trucks/SUVs, extensive bodywork, jambs/engine bay, full color changes, custom/tri-stage colors, restoration work, and additional disassembly can increase the project total.</p></div>
