...
const paintPackages = [
  { id: "resale", name: "Resale Quality", price: "$800 baseline", label: "Budget appearance refresh", text: "An appearance-focused option for resale, trade-in, auction, or inventory vehicles where improving presentation matters more than extensive correction or show-level detail.", includes: ["Exterior appearance refresh", "Basic prep & masking", "Limited surface correction", "Uniform color/finish goal", "Final visual inspection"] },
  { id: "daily", name: "Daily Driver Refinish", price: "$1,200 baseline", label: "Clean & presentable", text: "A practical full-body refinish for a vehicle you use every day, built around a cleaner, more uniform appearance without show-car levels of disassembly and correction.", includes: ["Daily-driver exterior baseline", "Standard prep & masking", "Minor surface correction", "Appropriate paint system", "Final finish inspection"] },
  { id: "dealer", name: "Dealership Quality", price: "$3,500 baseline", label: "Factory-style appearance", text: "For customers who want the vehicle to present closer to a newer dealership or factory-style finish, with more preparation, disassembly, correction, and finish refinement.", includes: ["More extensive prep", "Additional trim/component removal as needed", "Improved panel & surface correction", "Higher finish refinement", "Finish polish/detail as required"] },
  { id: "show", name: "Show Quality", price: "$5,500 baseline", label: "Highest finish standard", text: "For enthusiast, restoration, weekend, and show-oriented vehicles where preparation and final appearance matter more than keeping the project at a basic repaint budget.", includes: ["Extensive prep & blocking", "Greater disassembly", "Detailed surface correction", "Premium finish attention", "Cut/polish & final refinement"] },
  { id: "consult", name: "Custom / Specialty", price: "Consultation required", label: "Built around the vehicle", text: "For projects outside the four baseline packages, including extensive bodywork, restoration, unusual materials, major color-change scope, custom finishes, specialty vehicles, or anything that needs a vehicle-specific plan.", includes: ["Vehicle-specific scope", "Custom prep/bodywork plan", "Color/material consultation", "Disassembly plan as needed", "Individual project estimate"] },
];

const paintExpectations = [
  "Fix up my daily driver — clean, presentable, and durable",
  "Resale / trade-in refresh — improve appearance without overbuilding it",
  "Dealership / factory-style finish — I want it to present like a newer car",
  "Enthusiast / weekend car — I care about the details",
  "Show-quality finish — I want the highest standard possible",
  "Restoration / custom build — this is a project vehicle",
  "Not sure — I want DG Autos to recommend the right level",
];

const paintBudgets = ["Under $1,200", "$1,200–$3,499", "$3,500–$5,499", "$5,500+", "Custom / consultation-priced project", "Financing will help determine my budget", "Not sure yet"];
...
function App() {
  ...
  const [selectedPaintPackage, setSelectedPaintPackage] = useState("daily");
  ...
  async function submitAppointment(event: FormEvent<HTMLFormElement>) {
    ...
    const notes = serviceType === "Full Body Paint Consultation" ? [
      `Paint package: ${String(data.get("paintPackage") || "Not selected")}`,
      `Customer expectation: ${String(data.get("paintExpectation") || "Not specified")}`,
      `Comfortable budget: ${String(data.get("paintBudget") || "Not specified")}`,
    ...
    try {
      ...
      form.reset(); setAppointmentService("Full Body Paint Consultation"); setSelectedPaintPackage("daily"); setAppointmentDate(""); setAvailableSlots([]);
  ...
  return <div className="site-shell">
    ...
    <main>
      <section id="home" className="hero"><div className="hero-overlay"/><div className="hero-content container"><p className="eyebrow">Houston full-body automotive paint specialist</p><h1>Give the whole car<br/><span>a new finish.</span></h1><p className="hero-copy">DG Autos now specializes in full-body paint jobs—from same-color repaints to complete color changes and restoration refinishing. We still handle mechanical repairs and selected collision work, but complete paint projects are now the center of what we do.</p><div className="hero-actions"><a className="button button-primary" href="#booking">Book a Paint Consultation</a><a className="button button-secondary" href="#paint">Explore Full Paint Jobs</a><a className="button button-secondary" href="#financing">Paint Job Financing</a><a className="button button-secondary" href={`tel:${phoneNumber}`}>Call {phoneDisplay}</a></div><div className="trust-row"><span><strong>Full-body</strong> paint specialty</span><span><strong>Baseline packages</strong> from $800</span><span><strong>Financing</strong> on approved estimates</span></div></div></section>

      <section id="paint" className="section paint-focus-section"><div className="container"><div className="section-heading paint-heading"><p className="eyebrow">Full-body paint projects</p><h2>Choose the result. We build the paint plan.</h2><p>Every complete paint job starts with an in-person consultation because prep, body condition, color choice, trim removal, jamb work, and finish expectations change the scope. Start with the project direction and quality level that sounds closest to what you want.</p></div><div className="paint-options-grid">{paintProjects.map((project,index) => <article className={`paint-option-card ${index===1?"paint-option-featured":""}`} key={project.title}><span>{String(index+1).padStart(2,"0")}</span><small>{project.label}</small><h3>{project.title}</h3><p>{project.text}</p><a href="#booking">Start with a consultation →</a></article>)}</div>
      <div className="paint-package-section"><div className="paint-package-heading"><p className="eyebrow">Planning packages</p><h3>Know the baseline before you book</h3><p>These prices are baseline planning figures only. They are designed to help you choose the quality level closest to your goals before we inspect the vehicle and build the final project estimate.</p></div><div className="public-package-grid">{paintPackages.map(pkg=><article key={pkg.id} className={`public-package-card ${pkg.id==="daily"?"featured":""}`}><small>{pkg.label}</small><h4>{pkg.name}</h4><strong>{pkg.price}</strong><p>{pkg.text}</p><ul>{pkg.includes.map(item=><li key={item}>{item}</li>)}</ul><a href="#booking">Choose this level →</a></article>)}</div><p className="package-disclaimer"><strong>Baseline pricing notice:</strong> The prices shown above are not guaranteed quotes. They are starting baselines only. Final pricing is determined after consultation and vehicle inspection and can change based on vehicle size, existing paint/body condition, dents, rust, peeling or failed coatings, bodywork, disassembly, jambs, color change, pearl/tri-stage/custom materials, restoration work, and the finish level requested. Anything outside the listed baseline scope is priced by consultation.</p></div>
      <div className="paint-scope-strip"><span>Body prep</span><span>Surface correction</span><span>Color & finish planning</span><span>Refinish</span><span>Reassembly</span><span>Final inspection</span></div></div></section>
      ...
      <section id="booking" className="section booking-section paint-booking-section"><div className="container booking-grid"><div className="booking-copy"><p className="eyebrow">Paint consultation booking</p><h2>Tell us what “finished” means to you before you arrive</h2><p>Choose the package closest to your expected spend, tell us what kind of result you want, and reserve a consultation. This gives us a much better picture of the project before we see the vehicle.</p><div className="booking-points"><div><strong>1</strong><span>Choose your baseline package & expected finish</span></div><div><strong>2</strong><span>Tell us the car's condition and your budget</span></div><div><strong>3</strong><span>Choose an inspection time</span></div></div><div className="paint-consultation-note"><strong>Every package price shown is a baseline—not a fixed quote.</strong><span>We inspect the vehicle before final pricing. Vehicle size, bodywork, rust, peeling paint, color changes, jambs, disassembly, specialty materials, and finish expectations can change the final total. Projects outside the listed scope are consultation-priced.</span></div><small>Appointments are available Monday–Saturday, 9:00 AM–5:00 PM. The appointment is for consultation/intake; paint-project completion time is quoted separately.</small></div><form className="appointment-form" onSubmit={submitAppointment}><div className="form-row"><label>Name<input name="appointmentName" required autoComplete="name" placeholder="Your name"/></label><label>Phone<input name="appointmentPhone" required type="tel" autoComplete="tel" placeholder="Best contact number"/></label></div><label>Email <span>(optional)</span><input name="appointmentEmail" type="email" autoComplete="email" placeholder="you@example.com"/></label><label>Vehicle<input name="appointmentVehicle" required placeholder="Year, make, and model"/></label><label>Service<select name="appointmentService" required value={appointmentService} onChange={event=>setAppointmentService(event.target.value)}>{appointmentServices.map(service => <option key={service}>{service}</option>)}</select></label>{appointmentService==="Full Body Paint Consultation"&&<div className="paint-intake"><div className="paint-intake-header"><span>Full-body paint details</span><small>These answers help us prepare for your consultation.</small></div>
      <fieldset className="package-picker"><legend>Which baseline package is closest to what you want?</legend><div className="package-picker-grid">{paintPackages.map(pkg=><label key={pkg.id} className={`package-pick ${selectedPaintPackage===pkg.id?"selected":""}`}><input type="radio" name="paintPackage" value={`${pkg.name} — ${pkg.price}`} checked={selectedPaintPackage===pkg.id} onChange={()=>setSelectedPaintPackage(pkg.id)}/><span><small>{pkg.label}</small><strong>{pkg.name}</strong><b>{pkg.price}</b><em>{pkg.text}</em></span></label>)}</div><small className="package-picker-note">All dollar amounts are baseline planning figures only. Final pricing follows consultation and vehicle inspection.</small></fieldset>
      <label className="form-field-wide">What are you expecting out of the paint job?<select name="paintExpectation" defaultValue={paintExpectations[0]}>{paintExpectations.map(expectation=><option key={expectation}>{expectation}</option>)}</select></label><label>Comfortable project budget<select name="paintBudget" defaultValue="$1,200–$3,499">{paintBudgets.map(budget=><option key={budget}>{budget}</option>)}</select></label><label>Paint project<select name="paintProject" defaultValue="Same-color repaint"><option>Same-color repaint</option><option>Full color change</option><option>Restoration / custom refinish</option><option>Not sure — I want recommendations</option></select></label><label>Current paint / body condition<select name="paintCondition" defaultValue="Faded / oxidized / clear coat failure"><option>Faded / oxidized / clear coat failure</option><option>Peeling / flaking paint</option><option>Dents / body damage before paint</option><option>Previous bodywork / mixed paint condition</option><option>Good body — mainly changing color</option><option>Not sure</option></select></label><label>Color / finish direction <span>(optional)</span><input name="paintColorGoal" placeholder="Keep factory color, black, pearl white, custom color, not sure..."/></label><label>Interested in financing?<select name="paintFinancingInterest" defaultValue="Maybe — show me options after the estimate"><option>Yes — I want financing options</option><option>Maybe — show me options after the estimate</option><option>No — not right now</option></select></label></div>}<div className="form-row"><label>Date<input name="appointmentDate" required type="date" min={localDateOffset()} max={localDateOffset(90)} value={appointmentDate} onChange={event=>setAppointmentDate(event.target.value)}/></label><label>Available time<select name="appointmentTime" required defaultValue="" disabled={!appointmentDate||loadingSlots}><option value="">{loadingSlots?"Loading times…":!appointmentDate?"Choose a date first":availableSlots.length?"Choose a time":"No times available"}</option>{availableSlots.map(slot => <option key={slot} value={slot}>{displaySlot(slot)}</option>)}</select></label></div><label>Anything else we should know? <span>(optional)</span><textarea name="appointmentNotes" rows={4} placeholder={appointmentService==="Full Body Paint Consultation"?"Body damage, peeling areas, trim concerns, custom ideas, deadline, or anything else about the paint project":"Damage, symptoms, concerns, insurance claim, or anything else we should know"}/></label><input className="booking-honeypot" tabIndex={-1} autoComplete="off" name="company" aria-hidden="true"/><button className="button button-primary" type="submit" disabled={booking||loadingSlots||!availableSlots.length}>{booking?"Reserving consultation…":appointmentService==="Full Body Paint Consultation"?"Reserve Paint Consultation":"Reserve Appointment"}</button>{appointmentMessage&&<p className="appointment-success">{appointmentMessage}</p>}{appointmentError&&<p className="appointment-error">{appointmentError}</p>}<small>Booking reserves an intake/consultation time. DG Autos will contact you if anything about the requested visit needs to change.</small></form></div></section>
